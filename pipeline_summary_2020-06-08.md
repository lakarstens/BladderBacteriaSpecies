...
![](../../../public_facing_git/sm_overall_activity.png)

# Data download

I searched the Silva database on 11/10/2019 for each type strain of the
Thomas-White dataset. Search parameters were the species name, SSU r132,
and the strain=\[T\] designation. I chose the longest sequence length
with the highest stats. For the species that had no hits, I used the
synonum, if
available.
...
The files were downloaded and saved in folder
arb-silva.de\_2019-11-10\_id739827. A copy of the file was made and
renamed ktw\_16s\_type\_strains\_2019-11-10.

# data prep

T-Coffee requires that the FASTA description line only contains the ID.
I wrote a script that selects only the accession numbers from the
sequences and writes to a new file. The script is in
/thesis/gather\_markers/ssu16s/bladder\_population/arb-silva.de\_2019-11-10\_id739827/cull\_downloads.py,
so don’t look for it in /thesis\_git. The basic function is:
...
Using the *E. coli* type strain EU014689.1.1541 for reference
coordinates
...
| Bukin 2019        | 16S\_BV2f    | AGT GGC GGA CGG GTG AGT AA                 | \(\rightarrow\) | 102         | MiSeq    |                                                                                 |
| Bukin 2019        | 16S\_BV3r    | …                                          | \(\leftarrow\)  | 497?        | MiSeq    | this sequence is exactly the same as BV2f, and isn’t a palindrome. Typo?        |
| Bukin 2019        | MiCSQ\_343FL | TAT GGT AAT TGT CTC CTA CGG RRS GCA GCA G  | \(\rightarrow\) | 344         | MiSeq    | no matches to MSA, assumed numbers are position of *E. coli* 16S sequence\[^1\] |
| Bukin 2019        | MiCSQ\_806R  | AGT CAG TCA GCC GGA CTA CNV GGG TWT CTA AT | \(\leftarrow\)  | 806         | MiSeq    | no matches to MSA, assumed numbers are position of *E. coli* 16S sequence       |
...
| paper | name      | sequence                | direction       | E. coli pos | platform | notes |
| ----- | --------- | ----------------------- | --------------- | ----------- | -------- | ----- |
| me    | v3\_579F  | THT TSS RCA ATG GRS GVA | \(\rightarrow\) | 359         | MiSeq    |       |
| me    | v3\_779R  | GKN SCR AGC STT RHY CGG | \(\leftarrow\)  | 536         | MiSeq    |       |
| me    | v6\_1183F | CCG CCT GGG GAS TAC GVH | \(\rightarrow\) | 875         | MiSeq    |       |
| me    | v6\_1410R | AGT CCC RYA ACG AGC GCA | \(\leftarrow\)  | 1083        | MiSeq    |       |
...
| Bukin 2019        | 16S\_BV2f-16S\_BV3r      | bbv          |
| Bukin 2019        | MiCSQ\_343FL-MiCSQ\_806R | b646         |
| Caporaso 2011     | F515-R806                | cap          |
| Graspeuntner 2018 | V3F-V4R                  | gras         |
| me                | v6\_1183F-v6\_1410R      | v6           |
| me                | v3\_579F-v3\_779R        | v3           |

Each primer set coordinates were used to extract the subsequences from
the multisequence
alignment.
...
I put all the BLCA commands for every database into a bash script. Run
all at once.

Moved the extracted\_16s\_regions\_2019-11-26 dir and
blca\_typestr\_alldb.bash to the Ubuntu side, ran it.
...
Next I had to make a map file for the type strain accesion numbers and
known taxonomy.

### typo\!

I just noticed a typo. `b646` should be `b343`. Leaving it for now, but
beware.
...
There were a couple lines where the script didn’t pull out the names
correctly, like `X96497.1.1483,Corynebacterium 1;Corynebacterium
coyleae` so cleaned that up by hand. Then ran through the synonyms. I
would like to say that nothing will happen, because I checked for
synonyms when I donwnloaded the type strains, but we’ll see.
...
`validate_match.py` compares the results of the taxonomy assigned by a
classifier to the known taxonomy held in a map file.
...
GmbH. If the synonym names are already the same, it’s because the DSMZ
included information down to the strain level. Strains are out of scope,
so entries like *Enterobacter cloacae cloacae* and *Enterobacter cloacae
dissolvens* are considered synonyms of *Enterobacter cloacae*.
Misspellings that don’t map to known synonyms are assigned as a
mismatch. I’ll fix that later, but know that unless you’re typing your
own taxonomy into fasta records, the chance of misspellings are low.
...
“Unavailable” just means there’s no information for that record.

Input is the formatted csv file as written by the python script
‘blcaout\_to\_csv.py’

    id,taxon,rank,confidence
    CP011325.22226.23780,Bacteria,domain,100.0

the –map\_file option takes a path to a file that can be used to map
accession numbers to taxonomy by newick\_tools.inhouse\_to\_species().
See below for the list of some existing map files. run the script like
this:
